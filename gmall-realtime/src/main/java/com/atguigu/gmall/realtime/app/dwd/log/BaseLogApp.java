...
//        source.print("source >>>>>>");
        // TODO 4. 数据清洗，转换结构
        // 4.1 定义错误侧输出流
//        OutputTag<String> dirtyStreamTag = new OutputTag<String>("dirtyStream", TypeInformation.of(String.class));
        ...
        SingleOutputStreamOperator<JSONObject> fixedStream = keyedStream.process(
                new KeyedProcessFunction<String, JSONObject, JSONObject>() {
                    ...
                    public void open(Configuration param) throws Exception {
                        super.open(param);
                        lastLoginDt = getRuntimeContext().getState(new ValueStateDescriptor<String>(
                    ...
                    public void processElement(JSONObject jsonObj, Context ctx, Collector<JSONObject> out) throws Exception {
                        ...
                        String dt = DateFormatUtil.toDate(Long.parseLong(ts)).substring(0, 10);

                        if ("1".equals(isNew)) {
                            if (lastLoginDate != null && lastLoginDate.equals(dt)) {
                                isNew = "0";
                                jsonObj.getJSONObject("common").put("is_new", isNew);
                            } else {
                                lastLoginDt.update(dt);
                            }
                        } else {
                            if (lastLoginDate == null || !lastLoginDate.equals(dt)) {
                                lastLoginDt.update(dt);
...
//        separatedStream.getSideOutput(errorTag).print("error$$$");
        ...
        String display_topic = "dwd_display_log";
        String action_topic = "dwd_action_log";
        String error_topic = "dwd_error_log";
